...
struct LauncherView: View {
    ...
    /// The default database written by Sign In & Sync (inside the app sandbox container).
    private var defaultDBURL: URL? {
        FileManager.default
    ...
    private var defaultDBExists: Bool {
        guard let url = defaultDBURL else { return false }
    ...
    private var actionCards: some View {
        VStack(spacing: 20) {
            // If a synced database already exists, offer a one-tap shortcut to open it.
            if defaultDBExists, let url = defaultDBURL {
                Button {
            ...
            HStack(spacing: 24) {
                ActionCard(
                    ...
                    description: "Load any scouting_troop.db from disk"
                ) {
                    openDatabase()
                }
                ActionCard(
    ...
    private func startSync() {
        ...
        // before the sandboxed process can read the file, and stop when done.
        let capturedCSVURL = csvURL
        let didStartAccess = capturedCSVURL?.startAccessingSecurityScopedResource() ?? false
        let capturedCSVPath = capturedCSVURL?.path

        Task {
            defer {
                if didStartAccess { capturedCSVURL?.stopAccessingSecurityScopedResource() }
            }
