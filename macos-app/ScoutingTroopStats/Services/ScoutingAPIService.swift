...
actor ScoutingAPIService {
    ...
    private static let authBase = "https://auth.scouting.org/api/users"
    ...
    func authenticate(username: String, password: String) async throws -> (token: String, userId: String?) {
        ...
        req.setValue("https://my.scouting.org", forHTTPHeaderField: "Origin")
        req.setValue("https://my.scouting.org/", forHTTPHeaderField: "Referer")
    ...
    private func perform(_ request: URLRequest) async throws -> Data {
        ...
        if http.statusCode >= 400 {
            let body = String(data: data, encoding: .utf8).map { String($0.prefix(500)) } ?? ""
            print("[ScoutingAPI] HTTP \(http.statusCode) from \(request.url?.absoluteString ?? "?"): \(body.isEmpty ? "<empty body>" : body)")
